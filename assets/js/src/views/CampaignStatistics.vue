...
export default {
  ...
  mounted() {
    this.granularity.current = this.granularity.options.day;
    this.getStatistics();
  ...
  methods: {
    ...
    chartPatch(date) {
      let { current, range } = this.granularity;
      const date_range = date ? date : this.messagesAt;
      this.chartFetch(current, date_range, data => {
        const { sent, seen, replied } = data.response.body;
        this.granularity.range = range.concat(
          sent.map(c => moment(c.dateTime).toDate())
        );

        this.messagesChart.$emit("dispatch", chart => {
          chart.load({
            columns: [
              ["x"].concat(this.granularity.range),
          ...
          chart.axis.min(
            new Date(Math.min.apply(null, this.granularity.range))
          );
          chart.axis.max(
            new Date(Math.max.apply(null, this.granularity.range))
          );
        });
      });
    },
    ...
    chartInitialize() {
      let { current, range } = this.granularity;
      this.chartFetch(current, null, data => {
        const { sent, seen, replied } = data.response.body;
        range = sent.map(c => moment(c.dateTime).toDate());
    ...
    chartFetch(granularity, range, onSuccess) {
      this.chartFetching = true;
      const { campaignId, accountId } = this.$route.params;
      const [begin, end] = range ? range : this.messagesAt;
      axios({
        ...
        params: {
          ...
          //campaignId: "5d7912e859e701120526a5b7"
        }
      })
        .then(({ data }) => {
          if (onSuccess) {
    ...
    getStatistics() {
      ...
      this.threads = null;
      //filters.categories.in.push(campaignId);

      axios({
        url: `${dh.apiUrl}/api/1.0.0/${
          dh.userName
        }/thread/list/ig_account/${accountId || 14}/${status}`,
        method: "post",
        data: { paging, ...filters }
      }).then(({ data }) => {
    ...
    chartModel(builder) {
      const self = this;
      const b = builder();
      const dates = b.x.slice(1);
      let d_min = this.getMinOrDefault(dates);
      let d_max = this.getMaxOrDefault(dates);
      let ranges = [d_min, d_max];
      ...
      return {
        ...
        zoom: {
          ...
          onzoom: function(domain) {
            const { current } = self.granularity;
            clearTimeout(self.zoomTimeout);
            ranges = domain;
            let callback = null;

            const [begin, end] = domain;
            const difference = moment(end).diff(begin, "days");

            if (Math.abs(difference) < 3) {
              callback = () => {
                self.granularity.current = options.hour;
                ...
                self.chartPatch(self.granularity.date);
              };
            } else {
              callback = () => {
                const prev = self.granularity.current;
                self.granularity.current = options.day;
                self.granularity.date = domain;
                if (prev != options.day) {
                  self.chartPatch(domain);
                }
              };
            }
            self.zoomTimeout = setTimeout(callback, 500);
          }
        },
        transition: {
          duration: 1000
